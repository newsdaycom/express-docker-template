...
import { sendRedisCommand } from './redis_streams.mjs';
...
export class RedisDelayedScheduler {
  ...
  async schedule(record) {
    ...
    await sendRedisCommand(this.client, 'zadd', 'ZADD', [
      this.scheduledSet,
      record.dueAt,
      serialized
    ]);
  ...
  async dueRecords(now = Date.now(), limit = 100) {
    const records = await sendRedisCommand(
      this.client,
      'zrangebyscore',
      'ZRANGEBYSCORE',
      [this.scheduledSet, '-inf', now, 'LIMIT', 0, limit]
    );
  ...
  async promoteDue(now = Date.now(), limit = 100) {
    ...
    for (const serialized of records) {
      ...
      if (locked) {
        try {
          const streamId = await sendRedisCommand(this.client, 'xadd', 'XADD', [
            record.targetStream,
            '*',
            'payload',
          ...
          await sendRedisCommand(this.client, 'zrem', 'ZREM', [
            this.scheduledSet,
            serialized
          ]);
