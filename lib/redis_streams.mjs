...
/**
 * Execute a Redis command through either node-redis convenience methods or a
 * lower-level `sendCommand` array interface.
 *
 * @param {object} client Redis-compatible client.
 * @param {string} method Convenience method name.
 * @param {string} command Redis command name.
 * @param {Array<string|number>} args Redis command arguments.
 * @returns {Promise<*>} Redis command response.
 */
export function sendRedisCommand(client, method, command, args) {
...
export class RedisStreamProducer {
  ...
  async publish(payload, fields = {}) {
    return sendRedisCommand(this.client, 'xadd', 'XADD', [
      this.stream,
      '*',
      'payload',
...
export class RedisStreamConsumer {
  ...
  async ensureGroup() {
    try {
      await sendRedisCommand(this.client, 'xgroup', 'XGROUP', [
        'CREATE',
        this.stream,
        this.group,
        '0',
  ...
  async acknowledge(ids) {
    ...
    return sendRedisCommand(this.client, 'xack', 'XACK', [
      this.stream,
      this.group,
      ...idsToAck
    ]);
  ...
  async recoverPending() {
    ...
    const pending = await sendRedisCommand(
      this.client,
      'xpending',
      'XPENDING',
      [this.stream, this.group, '-', '+', this.batchSize]
    );
    ...
    const claimed = await sendRedisCommand(this.client, 'xclaim', 'XCLAIM', [
      this.stream,
      this.group,
      this.consumer,
      this.pendingIdleMs,
